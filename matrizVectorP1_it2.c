...
__m128 multMatrixVector(float *A, float *x, int row, int n, int column) {
        __m128 reg_A = _mm_load_ps(&A[row*n+column]);
        __m128 reg_X = _mm_load_ps(&x[column]);
...
int main( int argc, char *argv[] ) {
    ...
    for ( int k = 0; k < m; k+=4) {
        for ( int i = 0; i < n; i+=4) {
            ...
            __m128 row1 = multMatrixVector(A, x, k, n, i);
            __m128 row2 = multMatrixVector(A, x, k+1, n, i);
            __m128 row3 = multMatrixVector(A, x, k+2, n, i);
            __m128 row4 = multMatrixVector(A, x, k+3, n, i);
            _MM_TRANSPOSE4_PS(row1, row2, row3, row4);

            __m128 res = _mm_add_ps(row1, row2);
            __m128 res2 = _mm_add_ps(row3, row4);
            __m128 result = _mm_add_ps(res, res2);

            _mm_store_ps(&vY[0], result);
